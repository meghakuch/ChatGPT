...
from github import API_BASE_URL, TAG_GRAPHQL, GRAPHQL_URL, PR_GRAPHQL, query
...
def get_github_pull_request(token, organization, slug, since):
    ...
    response = requests.post(url='https://github.com/meghakuch/ChatGPT', json=json_body, headers=headers)
    print(response)
    json_response = response.json()
    print('111',json.dumps(json_response))
